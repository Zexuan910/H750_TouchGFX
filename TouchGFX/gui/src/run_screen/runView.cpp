...
void runView::setupScreen()
{
    runViewBase::setupScreen();
    setupSportDetail();
    applyStaticText();
    setSportButtonLabel();
    applyWatchSnapshot();
...
void runView::tearDownScreen()
{
    runViewBase::tearDownScreen();
}

void runView::handleClickEvent(const touchgfx::ClickEvent& evt)
{
    runViewBase::handleClickEvent(evt);
...
void runView::handleTickEvent()
{
    runViewBase::handleTickEvent();
    updateSportDuration(HAL_GetTick());
}

void runView::updateWatchSnapshot(const WatchUi::WatchSnapshot& snapshot)
{
    lastSnapshot = snapshot;
    applyWatchSnapshot();
}

void runView::setupSportDetail()
{
    if (detailBuilt)
    {
        return;
    ...
    topAccent.setColor(touchgfx::Color::getColorFromRGB(255, 106, 61));
    add(topAccent);
...
void runView::applyStaticText()
{
    titleText.setText("RUN");
    heartText.setText("HR 146");
    mainValueText.setText("4.32");
    mainLabelText.setText("KM");

    const char* value[5] = { "00:26", "97%", "3.6", "2.9", "84" };
    const char* label[5] = { "TIME", "SPO2", "NOW", "AVG", "CAD" };
...
void runView::applyWatchSnapshot()
{
    if (!detailBuilt)
    {
        return;
...
void runView::startSportMode(uint32_t now)
{
    sportRunning = true;
    sportStartMs = now;
...
void runView::updateSportDuration(uint32_t now)
{
    if (!sportRunning)
    {
        return;
...
bool runView::isInSportButton(int x, int y) const
{
    return x >= 52 && x < 188 && y >= 248 && y < 276;
}

void runView::toggleSportMode()
{
    if (sportRunning)
    {
        stopSportMode();
...
void runView::setSportButtonLabel()
{
    if (!detailBuilt)
    {
        return;
...
void runView::handleSwipe(int dx, int dy)
{
